...
as the new stdout. **Branch stdout** differs from **branch
i2c-lcd-stdout** in this branch printf() will send the string to the
output chosen. Either LCD or PuTTY! A function pointer **stdout** allows
the output to be set to either lcd\_out or uart2\_out.

See [What does printf output do in an XC8
program?](https://microchipdeveloper.com/faq:29)
